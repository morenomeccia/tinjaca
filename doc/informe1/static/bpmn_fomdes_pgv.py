...
def add_event(G, node, xlabel, ev_type):
    G.add_node(node, label="", xlabel=fill(xlabel, 20), shape="circle", width="0.3",
               style=EV_STYLE[ev_type], fillcolor="white:grey")
...
def add_gateway(G, node, xlabel, gate_type):
    G.add_node(node, label="", image=GATE_ICON[gate_type], xlabel=xlabel, shape="diamond",
               style="filled", fillcolor="white:grey")
...
def add_cluster(G, name, label, cluster_nodes, cluster_edges):
    ...
    S.graph_attr.update(label=TITLE_TEMPLATE.format(label))
...
F.edge_attr.update(fontname="Liberation Sans Narrow Condensed")
